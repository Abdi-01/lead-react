...
import { getCategorySale } from '../../redux/action'
...
class ChartSection1 extends Component {
    ...
    componentDidMount() {
        this.props.getCategorySale()
    }

    getCategorySale = () => {
        Axios.get(API_URL + '/results/getCategorySale')
            .then((res) => {
                const { pieData, pieLabel } = this.state
                let count = 0
                res.data.map((val) => {
                    pieLabel.push(val.category)
                    count += val.qty
    ...
    render() {
        ...
        const dataPie = {
            labels: this.props.pieDataCategory,
            datasets: [
                {
                    data: this.props.pieDataQty,
            ...
            <MDBRow className="mb-4">
                ...
                <MDBCol md="4" className="mb-4">
                    <MDBCard className="mb-4">
                        ...
                        <MDBCardBody>
                            <Doughnut data={dataPie} height={300} options={{ responsive: true }} />
                        </MDBCardBody>
                    </MDBCard>
                    {/* <MDBCard className="mb-4">
                        <MDBCardBody>
...
export default connect(mapToProps, { getCategorySale })(ChartSection1);
