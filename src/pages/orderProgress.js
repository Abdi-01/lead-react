    ...
    return this.props.userTransReducer.map((item, index) => {
      return (
        <MDBCard key={item.id} style={{ marginTop: 5 }}>
          <MDBRow>
            <MDBCol sm='3'>
              <button className="invoiceTag" onClick={() => this.transactionDetail(item.invoice)} style={{ margin: 0, paddingLeft: 10, width: 210 }}>{item.invoice}</button>
            </MDBCol>
            <MDBCol sm='2'>
    ...
    return this.props.userTransReducer.map((val, index) => {
      if (this.state.selectedInvoice === val.invoice) {
        return (
          <MDBRow key={val.id}>
            {/* <MDBRow> */}
            <MDBCol md='9' ><p style={{ fontWeight: 'bold' }}>Shipping ({val.courier})</p></MDBCol>
            <MDBCol>IDR. {val.shippingPrice.toLocaleString()}</MDBCol>
            {/* </MDBRow> */}
            <MDBCol md='9'><p style={{ fontWeight: 'bold' }}>Amount</p></MDBCol>
            <MDBCol >IDR. {val.payment.toLocaleString()}</MDBCol>
            <MDBCol>
              Name : {val.username}
              <br />
              Address : {val.address}
...
export default connect(mapStateToProps, { getUserTransaction, getDetailTransaction })(TrackingPage);